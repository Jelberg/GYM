 ...
 * @author Elberg
 */
public class TestClaseFecha {
    Sql _conn = new Sql();
    ResultSet _rs;
    Gson _gson = new Gson();
    FOM05_Clase_Fecha _M05CF;
    Horario_Clase _hc;
...
"9999 );";
    ...
    @Test
    public void listaClasesTest(){
        try {
            String response = _M05CF.listaClases();
            assertNotNull (response);
        } catch (SQLException ex) {
            Logger.getLogger(TestClaseFecha.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    
    @Test
    public void getClassPTest(){
        String response = _M05CF.getHorario_Clase(9999);
        assertNotNull(response);
    }
}
